...
function Rule(pattern, target, behavior, credentials) {
    ...
    assert.equal(typeof target, 'string', 'target must be string')
    assert(
...
Rule.prototype.tryHandleWebSocket = function tryHandle(req, socket, head, href) {
    const target = this.match(href)

    if (!target)
        return

    this.handleWebSocket(req, socket, head, target)
    return true
}
...
Rule.prototype.handleWebSocket = function handle(req, socket, head, target) {
    switch (this.behavior) {
        case Rule.behavior.forward:
            forward.forwardWebSocket(req, socket, head, target)
            break
    }
}
