...
const statusCodes = require('http').STATUS_CODES,
      ...
      name = capitalize(manifest.name),
      ver = manifest.version,
      cache = {}

function generateErrorPage(status, message, description) {
    return (
        `<!DOCTYPE html>
        <html lang="en">
            <head>
                ...
                <title>We've got some trouble | ${status} - ${message}</title>
            ...
            <body>
                <div class="cover">
                    <h1>${message} <small>Error ${status}</small></h1>
...
function getErrorPage(status, message, description) {
    description = description || ''
    
    const key = description || '_'

    if (status in cache) {
        const group = cache[status]

        if (key in group)
            return group[ key ]
        else
            return group[ key ] = generateErrorPage(status, message, description)
    }
    else {
        const page = generateErrorPage(status, message, description)
        cache[ status ] = { [ key ]: page }
        return page
    }
}
...
function sendError(req, res, status, description) {
    ...
    if (accepts(req).type('html')) {
        type = 'text/html'
        body = getErrorPage(status, message, description)
    }
    else {
        type = 'text/plain'
        body = `${status} - ${description || message}`
    }
